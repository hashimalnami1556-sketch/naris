...
import { recordAudit, recordJobEvidence, getArtifacts, getAudit } from "./evidence.js";
import { registryStatus, getRegistryAsset, searchRegistry, scanUnrealW04 } from "./asset-registry.js";
...
const finalizeEvidence = async (jobId: string) => {
  const current = getJob(jobId);
  if (!current) return null;
  const artifacts = current.status === "SUCCEEDED" ? await recordJobEvidence(current) : [];
  const audit = await recordAudit(current, artifacts);
...
const enqueue = async (operation: string, parameters: Record<string, unknown>, assetId: string | undefined, permission: NarisPermission) => {
  ...
  const evidence = await finalizeEvidence(job.jobId);
  return { job: getJob(job.jobId), evidence };
};
...
app.registerTool("naris.asset.list", { description: "List repository asset files.", inputSchema: { prefix: z.string().optional(), limit: z.number().int().min(1).max(200).default(50) } }, async ({ prefix, limit }) => { const root = path.join(REPO, prefix ?? "assets"); const files: string[] = []; const walk = async (dir: string): Promise<void> => { if (files.length >= limit) return; try { for (const e of await fs.readdir(dir, { withFileTypes: true })) { if (files.length >= limit) break; const full = path.join(dir, e.name); if (e.isDirectory()) await walk(full); else files.push(path.relative(REPO, full).replaceAll(path.sep, "/")); } } catch {} }; await walk(root); return out({ root: prefix ?? "assets", count: files.length, files }); });
app.registerTool("naris.asset.search", { description: "Search asset paths by text.", inputSchema: { query: z.string().min(1), prefix: z.string().optional(), limit: z.number().int().min(1).max(100).default(50) } }, async ({ query, prefix, limit }) => { const root = path.join(REPO, prefix ?? "assets"); const files: string[] = []; const needle = query.toLowerCase(); const walk = async (dir: string): Promise<void> => { if (files.length >= limit) return; try { for (const e of await fs.readdir(dir, { withFileTypes: true })) { if (files.length >= limit) break; const full = path.join(dir, e.name); const rel = path.relative(REPO, full).replaceAll(path.sep, "/"); if (rel.toLowerCase().includes(needle)) files.push(rel); if (e.isDirectory()) await walk(full); } } catch {} }; await walk(root); return out({ query, count: files.length, files }); });
app.registerTool("naris.asset.get", { description: "Read asset metadata from known registries.", inputSchema: { assetId: z.string().min(1) } }, async ({ assetId }) => { const candidates = ["data/MASTER_ASSET_REGISTRY.json", "data/assets.json", "assets/registry.json"]; for (const file of candidates) { const data = await readJson(file); const items = Array.isArray(data) ? data : Array.isArray(data?.assets) ? data.assets : []; const found = items.find((x: any) => x?.asset_id === assetId || x?.assetId === assetId || x?.id === assetId); if (found) return out({ found: true, registry: file, asset: found }); } return out({ found: false, assetId, searched_registries: candidates }); });
app.registerTool("naris.registry.status", { description: "Validate and summarize the canonical NARIS master asset registry.", inputSchema: {} }, async () => { requirePermission("asset.read"); return out(await registryStatus()); });
app.registerTool("naris.registry.search", { description: "Search canonical asset metadata across the NARIS master registry.", inputSchema: { query: z.string().min(1), limit: z.number().int().min(1).max(100).default(50) } }, async ({ query, limit }) => { requirePermission("asset.read"); const assets = await searchRegistry(query, limit); return out({ query, count: assets.length, assets }); });
app.registerTool("naris.registry.get", { description: "Get one canonical asset from the NARIS master registry.", inputSchema: { assetId: z.string().min(1) } }, async ({ assetId }) => { requirePermission("asset.read"); const asset = await getRegistryAsset(assetId); return out({ found: Boolean(asset), assetId, asset }); });
app.registerTool("naris.unreal.w04.status", { description: "Inspect W04 Unreal project evidence without claiming an Unreal build or playtest.", inputSchema: {} }, async () => { requirePermission("project.read"); return out(await scanUnrealW04()); });
...
app.registerTool("naris.job.status", { description: "Return a NARIS job record plus persisted evidence.", inputSchema: { jobId: z.string().min(1) } }, async ({ jobId }) => out({ job: getJob(jobId) ?? null, artifacts: await getArtifacts(jobId), audit: await getAudit(jobId) }));
app.registerTool("naris.job.cancel", { description: "Cancel a queued/running/approval-required job.", inputSchema: { jobId: z.string().min(1) } }, async ({ jobId }) => { requirePermission("asset.build"); const job = cancelJob(jobId); if (job) await recordAudit(job, []); return out(job ?? { jobId, found: false }); });
app.registerTool("naris.report.generate", { description: "Generate repository and runtime evidence summary.", inputSchema: { assetId: z.string().optional() } }, async ({ assetId }) => out({ report_type: "evidence_bound", assetId: assetId ?? null, generated_at: new Date().toISOString(), evidence: { readme: await has("README.md"), integration: await has("docs/INTEGRATION_ARCHITECTURE.md"), naming: await has("docs/ASSET_NAMING.md"), mcp_spec: await has("mcp/SPEC.md"), artifacts: await getArtifacts(undefined, assetId), audit: await getAudit(undefined, assetId) }, runtime_claims: "ONLY_WHEN_BACKED_BY_WORKER_EVIDENCE" }));
app.registerTool("naris.artifact.list", { description: "List persisted runtime artifacts/evidence.", inputSchema: { jobId: z.string().optional(), assetId: z.string().optional() } }, async ({ jobId, assetId }) => out(await getArtifacts(jobId, assetId)));
app.registerTool("naris.audit.list", { description: "List audit records without secrets.", inputSchema: { jobId: z.string().optional(), assetId: z.string().optional() } }, async ({ jobId, assetId }) => out(await getAudit(jobId, assetId)));
...
app.registerTool("naris_qa_report", { description: "Evidence-backed QA metadata.", inputSchema: {} }, async () => out({ runtime_validation_required: true, mcp_spec: await has("mcp/SPEC.md"), artifact_registry: true, audit_log: true }));
