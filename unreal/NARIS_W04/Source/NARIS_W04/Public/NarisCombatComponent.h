...
public:
    ...
    FNarisCombatEvent OnCombatResolved;

    UFUNCTION(BlueprintCallable, Category="NARIS|Combat")
    FNarisCombatResult ResolveHit(
        float BaseDamage,
        float PoiseDamage,
        bool bParried,
        bool bCritical
    ...
    UFUNCTION(BlueprintCallable, Category="NARIS|Combat")
    FNarisCombatResult ResolveIncomingHit(
        float BaseDamage,
        float PoiseDamage,
        bool bCritical
    );
    ...
    UFUNCTION(BlueprintCallable, Category="NARIS|Combat")
    void OpenParryWindow(float DurationSeconds);

    UFUNCTION(BlueprintCallable, Category="NARIS|Combat")
    void OpenInvulnerabilityWindow(float DurationSeconds);

    UFUNCTION(BlueprintCallable, Category="NARIS|Combat")
    void ClearDefenseWindows();
...
private:
    float ParryWindowEndTime = -1.f;
    float InvulnerabilityEndTime = -1.f;
};
