...
T38 W04 material production gate maps every Ashen Forest material-library ID to a canonical NARIS-W04-MAT asset and deterministic Unreal MaterialInstanceConstant path; M_MASTER_SURFACE must expose BaseColor/Roughness/Metallic/NormalStrength/HeightRange/Emissive parameters with opaque blending, M_MASTER_WATER must expose Roughness/Refraction/NormalLayers with translucent blending; automation may create/update instances but must not fabricate simplified master materials, and Shipping is blocked on unresolved masters/instances, missing master capabilities or PBR parameter drift.
